...
export function usePDFExport() {
  ...
  const generatePDF = async (config: PDFReportConfig, data: any) => {
    try {
      // Determine export format - default to xlsx
      const format = config.format || 'xlsx';
      
      toast({
        title: "Generating Report",
        description: `Your ${format === 'json' ? 'JSON' : 'Excel (.xlsx)'} report is being prepared...`,
        variant: "default"
      });
      ...
      if (format === 'json') {
        ...
        // Excel format (.xlsx) - export ALL raw data from the database
        ...
        // Sheet 2: Export raw data from the database
        ...
        if (Array.isArray(data.employees)) {
          objectsToSheet('Employees', data.employees);
        }
      ...
      toast({
        title: "Success",
        description: `Report exported as ${format === 'json' ? 'JSON' : 'Excel (.xlsx)'} successfully`,
        variant: "default"
      });
    } catch (error) {
